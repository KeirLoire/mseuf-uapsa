...
(function($){		
	$.fn.jqte = function(options){
		...
		addParams('ol','insertorderedlist','Â¾',["ol"],true); // ordered list --> ctrl + .(dot)
		addParams('ul','insertunorderedlist','Â¼',["ul"],true); // unordered list --> ctrl + ,(comma)
		...
		return this.each(function(){
			...
			function styleFieldSwitch(styleField,status)
			{
				...
				if(status)
				{
					toolbar.data(mainData["d"],true); // stil seÃ§me alanÄ±nÄ±n aÃ§Ä±ldÄ±ÄŸÄ±nÄ± belirten parametre yaz 
					mainData["f"].slideDown(100); // stil seÃ§me alanÄ±nÄ± aÃ§
